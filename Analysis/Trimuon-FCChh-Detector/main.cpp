...
inline Cut ProcessEvent(const DelphesReader& delphesReader, TreeWriter& treeWriter) {
  ...
  /* ----- Muons ----- */
  if (delphesReader.Muon.GetSize() < 3) {
  ...
  size_t nMuons{0};
  const Muon* muons[3]{nullptr, nullptr, nullptr};  // Three highest PT muons
  ...
  for (const Muon& muon : delphesReader.Muon) {
    ...
    if (muons[0] == nullptr || muon.PT > muons[0]->PT) {
      ...
      muons[2] = muons[1];
      muons[1] = &muon;
    } else if (muons[2] == nullptr || muon.PT > muons[2]->PT) {
      muons[2] = &muon;
    }
  ...
  //
  // if (nJets != 0) {
  //   LogCut(ToString(Cut::muonTopology), "{}/{} jets", nJets,
  //   delphesReader.Jet.GetSize());
  //
